...
ex = Experiment('ExtraTreesRegressorRun')
@ex.config
def my_config():
    output_path = "ExtraTreesRegressor"
    datadir = 'input'
    n_folds = 10
    clfparams = {
             'bootstrap': False,
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'stam':
        ex.observers.append(MongoObserver.create(url='login1:27017',db_name = "allstateET"))
    else:
        ex.observers.append(MongoObserver.create(db_name = "allstateET"))
    run = ex.run()  
