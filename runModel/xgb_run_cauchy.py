...
ex = Experiment('xgbRunCauchyObjective')
@ex.config
def my_config():
    output_path = "xgbRunCauchyObjective"
    datadir = 'input'
    n_folds = 10
...
def experiment(output_path,
    ...
    # Optionally save oob predictions
        if save_oob_predictions:
            filename = '{}_oob_pred_cauchyobj.csv'.format(time)
    ...
    if save_test_predictions:
        filename = '{}_test_pred_cauchyobj.csv'.format(time)
